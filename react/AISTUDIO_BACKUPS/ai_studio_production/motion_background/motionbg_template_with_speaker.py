...
import subprocess
import random
from motion_background.scrapper import ai_scrapper
...
#     from motion_background.video_segmentation import segmentation_parallel_production
#     try:
...
def get_SpeakerSegmented_video(input_video_path, output_video_path, start_time, end_time):
    ...
    try:
        command = [ 
            'python', 'motion_background/video_segmentation/segmentation_parallel_production.py', 
...
def main_workflow(input_json_3words, input_json_8words, GIF_dataset_path, output_3words_json_path,
                  embedding_model):
    
    # Load the JSON file
    segments_data = load_json_file(input_json_3words)
    gifs_dataset = load_json_file(GIF_dataset_path)

    # Prepare data for embedding
    prepared_gifs_data = prepare_GIFs_dataset_for_embedding(gifs_dataset)

    # Calculate embeddings
    gifs_embeddings = calculate_dataset_embeddings(prepared_gifs_data, embedding_model)

    # Process segments (Change 'is_portrait' based on your video orientation)
    is_portrait = False  # or False for landscape
    updated_segments_data = update_with_gifs(segments_data, embedding_model, prepared_gifs_data, gifs_embeddings)

    status = save_json_file(output_3words_json_path, updated_segments_data)
    
    print(f"Updated data has been written to {output_3words_json_path}")
    ...
    segment_8words = load_json_file(input_json_8words)
...
def trigger_segmentation(filtered_segments, segment_8words, input_video_path, combined_output_json_path):
    ...
    status = save_json_file(combined_output_json_path, combined_data)
    
    print(f"Combined data has been written to {combined_output_json_path}")
    
    return (200, combined_data)

def trigger_main_func_and_segmentation(input_json_3words, input_json_8words, GIF_dataset_path, output_3words_json_path,
                    embedding_model, input_video_path, combined_output_json_path):
    try: 
        filtered_segments, segment_8words = main_workflow(input_json_3words, input_json_8words, GIF_dataset_path, output_3words_json_path,
                    embedding_model)

        status, combined_data = trigger_segmentation(filtered_segments, segment_8words, input_video_path, combined_output_json_path)
    
        return (status, combined_data)
    
    except Exception as e:
        print("Error occured in template bg:", str(e))
        return (409, None)



# #################### Test the functions ####################
# def main_workflow(input_json_3words, input_json_8words, input_video_path, GIF_dataset_path, output_json_path):
#     ## load the data
#     project_folder_path = os.path.dirname(os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__)))))
#     inputData_folder = os.path.join(project_folder_path, "public/data")
#     GIF_dataset_path = os.path.join(project_folder_path, "public/aistudio_json_data/GIFs_updated_dataset.json")
#     file_3words = "steve_3words.json"
#     file_3words_path = os.path.join(inputData_folder, file_3words)
    
#     # Load the JSON file
#     segments_data = load_json_file(file_3words_path)
#     gifs_dataset = load_json_file(GIF_dataset_path)
...
# def trigger_main_func_and_segmentation():
#     try: 
#         filtered_segments, segment_8words = main_workflow()
#         status, combined_data = trigger_segmentation(filtered_segments, segment_8words)
